...
#if defined(NDEBUG)
# error "Zinccoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa8988ac5aa23c748cf05547ce2ef7fe212eff4e2204e5f8313a85750eff88bf3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Zinccoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Zinccoin Signed Message:\n";
...
{
    // Zinccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Zinccoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Zinccoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Zinccoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Zinccoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Zinccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa8988ac5aa23c748cf05547ce2ef7fe212eff4e2204e5f8313a85750eff88bf3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is when zinccoin was born.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xc228d2d0c9ed34daffc48f3f3978032bddcbe8dcd2c57b0b37549128b23fb123"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Zinccoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZinccoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZinccoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZinccoinMiner(CWallet *pwallet)
{
    printf("ZinccoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zinccoin-miner");
    ...
    try { loop {
        ...
        printf("Running ZinccoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZinccoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZinccoinMiner, pwallet));
}
